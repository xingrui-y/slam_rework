...
struct MeshEngine
{
    __device__ inline void checkBlocks()
    {
        ...
        uint val = 0;
        if (x < mapState.maxNumHashEntries && map.hashEntries[x].next >= 0)
        {
            int3 pos = map.hashEntries[x].pos * mapState.blockSize;
            scan = true;
            val = 1;
        }
    ...
    __device__ inline void MarchingCube()
    {
        ...
        if (*noTriangles >= mapState.maxNumMeshTriangles || x >= *noBlocks)
            return;
        ...
        int3 pos = blockPos[x] * mapState.blockSize;
        for (int i = 0; i < mapState.blockSize; ++i)
        {
            ...
            {
                int tid = offset + i;
                if (tid >= mapState.maxNumMeshTriangles)
                    return;

                vertices[tid * 3 + 0] = vlist[triangleTable.ptr(cubeIdx)[i * 3 + 0]] * mapState.voxelSize;
                vertices[tid * 3 + 1] = vlist[triangleTable.ptr(cubeIdx)[i * 3 + 1]] * mapState.voxelSize;
                vertices[tid * 3 + 2] = vlist[triangleTable.ptr(cubeIdx)[i * 3 + 2]] * mapState.voxelSize;
                ...
                color[tid * 3 + 0] = clist[triangleTable.ptr(cubeIdx)[i * 3 + 0]];
                color[tid * 3 + 1] = clist[triangleTable.ptr(cubeIdx)[i * 3 + 1]];
                color[tid * 3 + 2] = clist[triangleTable.ptr(cubeIdx)[i * 3 + 2]];
            }
        }
    }
    ...
    uint *noBlocks;
    uint *noTriangles;
    cv::cuda::PtrStep<int> triangleTable;
};
...
uint MeshScene(cv::cuda::GpuMat &noOccupiedBlocks,
               cv::cuda::GpuMat &noTotalTriangles,
               const MapStruct &map,
               const cv::cuda::GpuMat &edgeTable,
               const cv::cuda::GpuMat &vertexTable,
               const cv::cuda::GpuMat &triangleTable,
               cv::cuda::GpuMat &normal,
               cv::cuda::GpuMat &vertex,
               cv::cuda::GpuMat &color,
               cv::cuda::GpuMat &blockPoses)
{
    noOccupiedBlocks.setTo(cv::Scalar(0));
    noTotalTriangles.setTo(cv::Scalar(0));

    MeshEngine engine;
    engine.map = map;
    engine.triangleTable = triangleTable;
    engine.edgeTable = (int *)edgeTable.ptr(0);
    engine.vertices = (float3 *)vertex.ptr(0);
    engine.noBlocks = (uint *)noOccupiedBlocks.ptr(0);
    engine.noTriangles = (uint *)noTotalTriangles.ptr(0);
    engine.normals = (float3 *)normal.ptr(0);
    engine.color = (uchar3 *)color.ptr(0);
    engine.blockPos = (int3 *)blockPoses.ptr(0);
    engine.noVertexTable = (int *)vertexTable.ptr(0);

    dim3 thread(MAX_THREAD);
    dim3 block(div_up(currentState.maxNumHashEntries, thread.x));
    ...
    cv::Mat host_data;
    noOccupiedBlocks.download(host_data);
    if (host_data.at<uint>(0, 0) <= 0)
        return 0;

    thread = dim3(8, 8, 1);
    block = dim3(div_up(host_data.at<uint>(0, 0), 16), 16, 1);

    MeshSceneKernel<<<block, thread>>>(engine);
    safe_call(cudaGetLastError());
    safe_call(cudaDeviceSynchronize());

    noTotalTriangles.download(host_data);
    int no = min(host_data.at<uint>(0, 0), currentState.maxNumMeshTriangles);
